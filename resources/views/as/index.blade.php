...
<head>
    <meta charset="UTF-8">
    <title>Data Accessoris dan Stabilizier</title>
    ...
    if (submenu.style.display === 'none' || submenu.style.display === '') {
      ...
      <div class="table-title">
        <div class="text" style="margin-left: 60px;">
          <h1>Data Accessoris dan Stabilizier</h1>
          <a href="{{ route('as.create') }}" class="btn-tambah">Tambah +</a>
        </div>
      </div>
      <div class="container">
    <br><br>
    @if(count($dataAccessorisStabiliziers) > 0)
    <table class="table">
        <thead>
            <tr>
                <th>Nama Barang</th>
        ...
        <tbody>
            @foreach($dataAccessorisStabiliziers as $dataAccessorisStabilizier)
            <tr>
                <td>{{ $dataAccessorisStabilizier->accessoris_stabilizier }}</td>
                <td>{{ $dataAccessorisStabilizier->harga}}</td>
                <td>
                   <img src="{{ asset('storage/' . $dataAccessorisStabilizier->gambar) }}" alt="" srcset="" width="80" height="80">
                </td>
                <td>{{ $dataAccessorisStabilizier->tanggal_mulai }}</td>
                <td>{{ $dataAccessorisStabilizier->tanggal_berakhir }}</td>
                <td>{{ $dataAccessorisStabilizier->status_ketersediaan }}</td>
                <td>
                    <a href="{{ route('as.edit', ['id' => $dataAccessorisStabilizier->id]) }}" >
                    ...
                    <form action="{{ route('as.destroy',['id' => $dataAccessorisStabilizier->id]) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
