    ...
    if (submenu.style.display === 'none' || submenu.style.display === '') {
      ...
      <div class="table-title">
        <div class="text" style="margin-left: 60px;">
          <h1>Data Lensa</h1>
          <a href="{{ route('lensa.create') }}" class="btn-tambah">Tambah +</a>
        </div>
      </div>
      <div class="container">
    <br><br>
    @if(count($dataLensas) > 0)
    <table class="table">
        <thead>
            <tr>
                <th>Nama Barang</th>
        ...
        <tbody>
            @foreach($dataLensas as $dataLensa)
            <tr>
                <td>{{ $dataLensa->lensa }}</td>
                <td>{{ $dataLensa->harga_lensa }}</td>
                <td>
                <img src="{{ asset('storage/' . $dataLensa->gambar_lensa) }}" alt="" srcset="" width="80" height="80">
                </td>
                <td>{{ $dataLensa->tanggal_mulai }}</td>
                <td>{{ $dataLensa->tanggal_berakhir }}</td>
                <td>{{ $dataLensa->status_ketersediaan }}</td>
                <td>
                    <a href="{{ route('lensa.edit', ['id' => $dataLensa->id]) }}" >
                    ...
                    <form action="{{ route('lensa.destroy',['id' => $dataLensa->id]) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
